...
async function editBranch(i){const {x}=await active(),m=x?.messages?.[i];if(!m)return;const t=prompt('Edit this message in a new branch',m.text);if(t===null||!t.trim())return;await makeBranch(i,'edit',t);await render();if(m.role==='user'){const b=document.querySelector('#forgeContinue');if(b)b.click()}else baseRegen(i)}
function baseRegen(i){const b=document.querySelector(`[data-regen="${i}"]`);if(!b)return;const base=b.__nfBase;if(typeof base==='function')base.call(b,new Event('click'));else b.click()}
async function regenBranch(i){const {x}=await active(),m=x?.messages?.[i];if(!m||m.role!=='assistant')return;await makeBranch(i,'branch');await render();baseRegen(i)}
